...
include_once("inc/Functions.php");
        ...
        $sql ="SELECT DISTINCT ward_no FROM assesment_input ORDER BY ward_no ASC";
        // $sqll = "SELECT owner_name,father_name,mother_name,ward_no,holding_no,previous_due,nid,imopsed_tax,mobile_no,village FROM assesment_input WHERE union_code='$unioncode' ORDER BY ward_no ASC";
...
$header_html = "<div style='text-align:center'>
        ...
        while($row_ward = $ass_res->fetch_assoc()){

          $ward_no = $row_ward['ward_no'];
          $sqlll = "SELECT owner_name,father_name,mother_name,ward_no,holding_no,previous_due,nid,imopsed_tax,mobile_no,village FROM assesment_input WHERE union_code='$unioncode'AND ward_no='$ward_no' ORDER BY holding_no ASC";
          $ward_resss = $addtax->con->query($sqlll);
          while($row_ward = $ward_resss->fetch_assoc()){
            
        $banglaserial = $addtax->bangla_convertor($serial);
        $ownername = $row_ward['owner_name'];
        $fathersname = $row_ward['father_name'];
        if($row_ward['mother_name']==""){
           $mothername = "-";
        }else{
          $mothername = $row_ward['mother_name'];
        }
        $village = $row_ward['village'];
        $mobileno = $addtax->bangla_convertor($row_ward['mobile_no']);
        $imposedtax = $addtax->bangla_convertor($row_ward['imopsed_tax']);
        $nid = $addtax->bangla_convertor($row_ward['nid']);
        $previous_due = $addtax->bangla_convertor($row_ward['previous_due']);
        $ward = $addtax->bangla_convertor($row_ward['ward_no']);
        $holding = $addtax->bangla_convertor($row_ward['holding_no']);
        $innertable .= "<tr>
